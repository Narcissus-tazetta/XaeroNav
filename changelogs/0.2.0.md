...
- Much better routing in dimensions with a ceiling, such as the Nether — routes that previously stopped short or never appeared should now be found.
...
- The HUD showing "arriving soon" / distance-remaining for intermediate waypoints instead of only the final destination.
- Route drawing on Xaero's World Map and Minimap, and the "Route here" right-click entries, not working on Forge (XaeroNav's Xaero integration was never registered with Mixin).
