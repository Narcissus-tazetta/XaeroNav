...
粗いルートを引き、そこへ向けて詳細探索を区間ごとに継ぎ足していきます。現世とエンドでは、
読み込み済みの範囲そのものを探索と同じ移動で組んだ航法グラフを裏で作り、粗いルートの中間目標へ
...
| `costToGoGuideEnabled` | `true` | 詳細探索に目的地までの残りコストの見積もりを使うか。現世・エンドでは読み込み済みの範囲から航法グラフを裏で組み、中間目標へ寄らず目的地をそのまま狙う（空いている CPU コアと 150〜250MB ほどのメモリを使う）。それ以外の場面と組み上がるまでは粗いルートの見積もりを使う。`false` で直線距離だけに戻る |
...
- 航法グラフが見るのは自分の周り 160 ブロックまでで、組み直す前のブロックの変化は反映されません。歩くたびと、探索が前進しなくなったときに組み直します。組むあいだは、16 ブロック歩くごとに空いている CPU コアが少しの間埋まります。
