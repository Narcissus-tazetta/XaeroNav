...
| `costToGoGuideEnabled` | `true` | 詳細探索に目的地までの残りコストの見積もりを使うか。読み込み済みの範囲から航法グラフを裏で組み、中間目標へ寄らず目的地をそのまま狙う（空いている CPU コアと、最大 460MB ほどのメモリを使う。ヒープ上限が 2.5GB 未満なら 260MB ほど）。組み上がるまでは粗いルートの見積もりを使う。`false` で直線距離だけに戻る |
