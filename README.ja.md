...
**[経路探索の仕組み →](docs/how-routing-works.ja.md)** 粗い地図から1歩ごとの移動まで、経路をどう見つけて
歩きながらどう保つかを図つきで説明しています。
...
長距離は段階に分けて解きます。読み込み済みチャンクの外については Xaero の地図データから
粗いルートを引き、そこへ向けて詳細探索を区間ごとに継ぎ足していきます。読み込み済みの範囲
そのものは探索と同じ移動で組んだ航法グラフを裏で作り、粗いルートの中間目標へ
寄り道せずに目的地をそのまま狙います。ネザーのように同じ XZ に
複数の階層が重なる次元でも、階層をまたぐ道筋を扱えます（詳しくは[経路探索の仕組み](docs/how-routing-works.ja.md)）。地下にいて目的地が地上の場合は、
...
| `costToGoGuideEnabled` | `true` | 詳細探索に目的地までの残りコストの見積もりを使うか。読み込み済みの範囲から航法グラフを裏で組み、中間目標へ寄らず目的地をそのまま狙う（空いている CPU コアと、最大 610MB ほどのメモリを使う。ヒープ上限が 2.5GB 未満なら 330MB ほど）。組み上がるまでは粗いルートの見積もりを使う。`false` で直線距離だけに戻る |
| `detailHorizonBlocks` | `96` | 詳細探索が一度に狙う最大の水平距離（ブロック）。これより遠い目的地には中間目標を挟む |
| `maxBridgeRunBlocks` | `96` | 空中に足場を置いて渡る橋を何マス連続させたら迂回に切り替えるか（`0` で無制限） |
...
| `elytraFlyingMinGroundClearanceBlocks` | `4` | エリトラの滑空を「飛んでいる」とみなす、足元から地面（水面を含む）までの最小の高さ（ブロック）。加えて0.5秒続くことも要るので、ジャンプで跳ねただけでは経路を捨てない |
...
- エリトラ経路では、HUD に飛行案内・目的地までの直線距離・この先の上昇量が表示されます。
...
- 航法グラフが見るのは自分の周り 224 ブロックまで（Java のヒープ上限が 2.5GB 未満なら 160 ブロック、
  ...
  なったときに組み直します。組み直すあいだは、8 ブロック歩くごとに CPU コアの半分ほどが最大 2 秒ほど埋まります。
- Java のヒープが 2GB 未満の環境は対象外です。ゲーム本体がメモリ不足で落ちることがあります。
