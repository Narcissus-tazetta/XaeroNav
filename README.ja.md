...
| `costToGoGuideEnabled` | `true` | 詳細探索に目的地までの残りコストの見積もりを使うか。読み込み済みの範囲から航法グラフを裏で組み、中間目標へ寄らず目的地をそのまま狙う（空いている CPU コアと 150〜250MB ほどのメモリを使う）。組み上がるまでは粗いルートの見積もりを使う。`false` で直線距離だけに戻る |
...
- 航法グラフが見るのは自分の周り 160 ブロックまでで、組み直す前のブロックの変化は反映されません。歩くたびと、探索が前進しなくなったときに組み直します。組むあいだは、8 ブロック歩くごとに空いている CPU コアが 1 秒ほど埋まります。
