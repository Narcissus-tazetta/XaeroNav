...
どの方法で指定しても、目的地は Xaero の地図上に示されます。線を端まで辿らなくても、どこへ向かって
いるのかが分かります。Xaero's Minimap を導入していれば **Xaero の一時ウェイポイント**として登録されるので、
回転するミニマップでも常に立った向きで出て、画面の外に出れば縁に寄って距離が付き、ワールド内にも
他のウェイポイントと同じように表示されます。ディスクには保存されず、経路をクリアすると消えます。
ミニマップが無い場合は、XaeroNav が世界地図に自前のピンを描きます（縮尺を変えても画面上の大きさは一定）。
...
`mapdata [radiusChunks]`（周辺で使える Xaero の地図データの量）、`route` と `corridor`（粗い中間目標列と
その区間ごとの精緻化）、`probe`（詳細探索がどこまで届き、なぜ止まったか）、`flight`（空中経路）。
意図と違う経路が出た理由を説明するためのものなので、不具合報告にはこの出力を添えてください。
...
| 🟡 黄褐 | 長距離ルートの粗い中間目標列 |
| 📍 赤いピン | 目的地（Xaero's Minimap が無い環境で XaeroNav が描くもの。縮尺を変えても画面上の大きさは一定） |
| 🔷 空色 | エリトラ滑空中の空中経路 |
...
- 地図連携は Xaero の内部へ差し込んで実現しています。新しい Xaero でその形が変わった場合は
  その部分だけが無効になります（何が無効になったかをワールド参加時に1度だけチャットで知らせます。
