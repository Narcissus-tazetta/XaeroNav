...
| 次元 | 狙う先 | ガイド | 重み |
|---|---|---|---|
| 天井の無い次元（現世・エンド） | 最終目的地 | 航法グラフ（`NavGraph` / `WindowField`） | 1.0 |
| 同上でガイドが使えない回 | 層1・層2の中間目標 | 区間の箱で組む層1 | 設定値 |
| 天井のある次元（ネザー） | 最終目的地 | 3D粗層（`VoxelCostToGo`） | 設定値 |

### 航法グラフ

読み込み済みの窓（プレイヤーから水平160ブロックの正方形、描画距離の方が狭ければそちら）の中を、
**層3とまったく同じ移動生成**で16³のセクションごとに辺へ落とし、目的地から逆Dijkstraして残りコストを作る。
窓の外へ出る辺の先と窓の縁には「外の推定」を置く。現世は層1、エンドと地図が無いときは目的地までの直線距離
（幾何下限）で、直線距離は目的地が窓の外にあるときだけ置く。
...
- ガイドは16ブロック歩くごとにワーカーで組み直し、組み上がるまでは直前のガイドか従来の探索で進む。
...
そこは航法グラフに任せます。歩き通しの模型（`NavGraphWalkBenchTest`、16〜32ブロック歩くごとの組み直し込み）で、
現世の広域長距離は1.067/1.165倍から1.016/1.030倍（洞窟始点のルートは1.029/1.058倍）、エンドは1.122倍・未到達1本から
1.013/1.029倍になりました（平均/最悪、全視界の最適経路比）。
