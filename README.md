...
エリトラで飛ぶ経路は `/xaeronav flyto <x> <y> <z>`。徒歩とは別のアルゴリズム（地形の上を越える
連続座標の経路）で、逸脱検知や自動再計算は行いません。エリトラ本体やロケット花火が無い場合は
その場で警告します。

経路を消すのは `/xaeronav clear` かキーバインド。徒歩とエリトラは同時には表示されません
（片方を指定するともう片方は消えます）。
...
| `groundLevelY` | `60` | この高さ以上を地上とみなす（地上優先ナビの基準） |
| `recalcIntervalTicks` | `40` | 経路上のブロック変化を確認する間隔 |
| `maxExpandedNodes` | `30000` | 1 回の探索で展開するノード数の上限。大きいほど遠くまで正確ですが CPU を使います |
