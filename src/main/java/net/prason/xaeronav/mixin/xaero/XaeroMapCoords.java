...
final class XaeroMapCoords {
     ...
     */
    static boolean isSameDimensionAsPlayer(ResourceKey<Level> mapDim, Level playerLevel) {
        return mapDim == null || mapDim == playerLevel.dimension();
    }

    /** 高さが分からない座標はプレイヤーと同じ高さを狙う（探索範囲は上下にも広がるため近くまでは届く）。 */
    static int resolveGoalY(int mapY, Player player) {
